...
export default function RiderLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await authService.loginRider(username, password);
      
      if (response.success) {
        // Redirect ke rider dashboard
    ...
    >
      <div className="flex flex-col items-center mb-8">
        <div className="w-12 h-12 bg-[#1e40af] rounded-lg flex items-center justify-center mb-4">
          <span className="material-symbols-outlined text-white" style={{ fontVariationSettings: "'FILL' 1" }}>two_wheeler</span>
        </div>
        <h2 className="text-[#1e40af] font-bold text-xl mb-8">Susu Gepuk | Rider</h2>
      </div>

      <div className="mb-8">
        <h3 className="text-2xl font-bold text-gray-900 mb-2">Login Rider</h3>
        <p className="text-gray-500 font-body-md">Silakan masuk ke aplikasi penjual Susu Gepuk.</p>
      </div>
      ...
      <form className="space-y-6" onSubmit={handleSubmit}>
        <div>
          ...
          <input
            ...
            placeholder="Masukkan username rider"
            disabled={loading}
            required
          />
        </div>
        <div>
          <div className="flex justify-between mb-2">
            ...
            <Link to="/rider/forgot" className="text-sm font-bold text-[#1e40af] hover:underline font-label-bold">Butuh bantuan?</Link>
          </div>
        ...
        <div className="flex items-start gap-2 bg-gray-50 p-3 rounded-lg">
          <span className="material-symbols-outlined text-[#1e40af] text-sm mt-0.5" style={{ fontVariationSettings: "'FILL' 1" }}>info</span>
          <p className="text-xs text-gray-600 leading-relaxed font-body-md">Login untuk mengakses aplikasi penjualan dan tracking lokasi real-time.</p>
        </div>
      ...
      <div className="mt-8 text-center text-sm font-body-md">
        ...
        <Link to="/admin/login" className="text-[#1e40af] font-bold hover:underline">Login sebagai Admin</Link>
      </div>

      <div className="mt-8 text-center text-sm font-body-md">
        <span className="text-gray-500">Belum terdaftar? </span>
        <Link to="/rider/register" className="text-[#1e40af] font-bold hover:underline">Daftar sekarang</Link>
      </div>

      <div className="mt-12 pt-6 border-t border-gray-100 flex flex-col items-center gap-2 text-xs text-gray-400 font-body-md">
        <p>© 2026 Susu Gepuk Rider Application.</p>
